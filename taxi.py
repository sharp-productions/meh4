...
class TaxiEnv(Env):
    ...
    ### Observations
    There are 720 discrete states since there are 36 taxi positions, 5 possible
    locations of the passenger (including the case when the passenger is in the
    taxi), and 4 destination locations.

    Note that there are 576 states that can actually be reached during an
    episode. The missing states correspond to situations in which the passenger
    is at the same location as their destination, as this typically signals the
    end of an episode. Four additional states can be observed right after a
    successful episodes, when both the passenger and the taxi are at the destination.
    This gives a total of 580 reachable discrete states.
    ...
    * v0: Initial versions release
    ...
    def __init__(self, map_name="5"):
        self.desc = np.asarray(MAPS[map_name], dtype="c")
        map_name = int(map_name)

        self.locs = locs = [(0, 0), (0, map_name - 1), (map_name - 1, 0), (map_name - 1, map_name - 2)]
        self.locs_colors = [(255, 0, 0), (0, 255, 0), (255, 255, 0), (0, 0, 255)]

        num_states = map_name * map_name * 5 * 4 # 500 or 720 or 980
        num_rows = map_name # 5
        num_columns = map_name # 5
        max_row = num_rows - 1
            ...
            for col in range(num_columns):
                for pass_idx in range(len(locs) + 1):  # +1 for being inside taxi
                    for dest_idx in range(len(locs)):
                        state = self.encode(row, col, pass_idx, dest_idx, map_name)
                        # print (state, "from ", row, col, pass_idx, dest_idx, map_name)
                        ...
                        for action in range(num_actions):
                            ...
                            new_state = self.encode(
                                new_row, new_col, new_pass_idx, dest_idx, map_name
                            )
    ...
    def encode(self, taxi_row, taxi_col, pass_loc, dest_idx, map_name):
        # (6) 6, 6, 4
        i = taxi_row
        i *= map_name
        i += taxi_col
    ...
    def decode(self, i, map_name):
        out = []
        out.append(i % 4)
        i = i // 4
        out.append(i % 5)
        i = i // 5
        out.append(i % map_name)
        i = i // map_name
        out.append(i)
        assert 0 <= i < map_name
        return reversed(out)
